...
namespace Opsive.Shared.Audio
{
    ...
    public class AudioConfig : ScriptableObject
    {
        ...
        protected virtual AudioSource AddAudioSource(AudioSourceGroup audioSourceGroup)
        {
            ...
            audioGameObject.name = m_ShareAudioSource ? "SharedAudioSource" : "ReservedAudioSource: " + name;
...
#if UNITY_EDITOR
        /// <summary>
        /// A menu item used to create audio configs by writing audio clips.
        /// </summary>
        [UnityEditor.MenuItem("Assets/Create/Opsive/Audio/One Audio Config From Selected Clips", false, 0)]
        public static void CreateOneAudioConfigFromSelectedClips()
        {
            var audioConfig = ScriptableObject.CreateInstance<AudioConfig>();
            var allObjects = UnityEditor.Selection.objects;
            var audioClips = new AudioClip[allObjects.Length];
            for (int i = 0; i < allObjects.Length; i++) {
                audioClips[i] = allObjects[i] as AudioClip;
            }

            audioConfig.AudioClips = audioClips;
            
            var audioClip = UnityEditor.Selection.activeObject as AudioClip;
            UnityEditor.ProjectWindowUtil.CreateAsset(audioConfig, $"{audioClip.name}.asset");
        }
 
        /// <summary>
        /// The condition checking that the selected asset is an Audio clip.
        /// </summary>
        /// <returns></returns>
        [UnityEditor.MenuItem("Assets/Create/Opsive/Audio/One Audio Config From Clips", true)]
        public static bool CreateOneAudioConfigFromSelectedClipsValidate()
        {
            var allObjects = UnityEditor.Selection.objects;
            if (allObjects.Length == 0) { return false;}

            for (int i = 0; i < allObjects.Length; i++) {
                if (!(allObjects[i] is AudioClip)) {
                    return false;
                }
            }
            return true;
        ...
        /// A menu item used to create audio configs by writing audio clips.
        /// </summary>
        [UnityEditor.MenuItem("Assets/Create/Opsive/Audio/Multi Audio Configs From Clips", false, 0)]
        public static void CreateMultipleAudioConfigsFromSelectedClips()
        {
            var allObjects = UnityEditor.Selection.objects;
            for (int i = 0; i < allObjects.Length; i++) {
                if (!(allObjects[i] is AudioClip audioClip)) {
                    return;
                }
                
                var audioConfig = ScriptableObject.CreateInstance<AudioConfig>();
                audioConfig.AudioClips = new[] { audioClip };
                UnityEditor.ProjectWindowUtil.CreateAsset(audioConfig, $"{audioClip.name}.asset");
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        [UnityEditor.MenuItem("Assets/Create/Opsive/Audio/Multi Audio Configs From Clips", true)]
        public static bool CreateMultipleAudioConfigsFromSelectedClipsValidate()
        {
            var allObjects = UnityEditor.Selection.objects;
            if (allObjects.Length == 0) { return false;}
