...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    public class Inventory : InventoryBase
    {
        ...
        [Tooltip("The runtime items that should be added when the inventory loads.")]
        [SerializeField] protected GameObject[] m_RuntimePickups;
        ...
        public GameObject[] RuntimePickups { get { return m_RuntimePickups; } set { m_RuntimePickups = value; } }
        ...
        protected override void Start()
        {
            ...
            // Add any runtime pickups immediately after the character has spawned.
            if (m_RuntimePickups != null) {
                for (int i = 0; i < m_RuntimePickups.Length; ++i) {
                    if (m_RuntimePickups[i] == null) {
                        continue;
                    }

                    var item = m_RuntimePickups[i].GetComponent<Item>();
                    if (item == null || HasItem(item)) {
                        continue;
                    }

                    Item.SpawnItem(m_GameObject, item);
                }
            }
        }
        ...
        protected override bool HasItemInternal(Item item)
        {
            ...
            return GetItemInternal(item.ItemDefinition as ItemType, item.SlotID) != null;
        }
