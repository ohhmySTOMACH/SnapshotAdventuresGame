...
namespace Opsive.UltimateCharacterController.FirstPersonController.Items
{
    ...
    public class FirstPersonShootableWeaponProperties : FirstPersonWeaponProperties, IShootableWeaponPerspectiveProperties
    {
        ...
        [NonSerialized] public float LookSensitivity { get { return m_LookSensitivity; } set { m_LookSensitivity = value; } }
                ...
                if (firstPersonObject != null) {
                    ...
                    if (objectIdentifiers.Length > 0) {
                        ...
                        if (m_FirePointAttachmentLocation == null && m_FirePointAttachmentLocationID != -1) {
                            Debug.LogWarning($"Warning: Unable to find the first person fire point attachment ObjectIdentifier with the ID {m_FirePointAttachmentLocationID} for item {name}.");
                        }
                        if (m_ReloadableClipAttachment == null && m_ReloadableClip != null && m_ReloadableClipAttachmentID != -1) {
                            Debug.LogWarning($"Warning: Unable to find the first person reload attachment ObjectIdentifier with the ID {m_ReloadableClipAttachmentID} for item {name}.");
                        }
                        if (m_ReloadProjectileAttachment == null && m_ReloadProjectileAttachmentID != -1) {
                            Debug.LogWarning($"Warning: Unable to find the first person reload projectile ObjectIdentifier with the ID {m_ReloadProjectileAttachmentID} for item {name}.");
                        }
                    }
                }
            }
